pipeline {
  ...
  environment {
	awsecrRegistry = '637423550581.dkr.ecr.ap-northeast-2.amazonaws.com/eks-repo'
  ...
  stages {
	...
	stage('Checkout Application Git Branch') {
  	steps {
    	checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/finalcloudproject4th/forProject.git']]])
  	}
  	// steps 가 끝날 경우 실행한다.
	...
	stage('Deploy') {
  	steps {
    	...
    	git credentialsId: githubCredential,
        	url: 'https://github.com/finalcloudproject4th/forProject.git',
        	branch: 'main'  
	...
	stage('Push to Git Repository') {
  	steps {
    	withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
      	sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/finalcloudproject4th/forProject.git"  	 
    	}
  	}
  	post {
            	failure {
                	echo 'K8S Manifest failure'
              	  slackSend (color: '#FF0000', message: "FAILED: K8S Manifest Update ${currentBuild.number}")
            	}
            	success {
                	echo 'K8S Manifest success'
              	  slackSend (color: '#0AC9FF', message: "SUCCESS: K8S Manifest Update ${currentBuild.number}")
            	}
        	}	 
	}
  }
